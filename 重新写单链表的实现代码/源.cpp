...
Status ChainList::createTail(ElemType *datas, ElemType length)  //单链表的整表创建,尾插法
{
...
/*Status ChainList::createHead(ElemType *datum, ElemType extent)  //写的头插法
{
...
ElemType ChainList::insertAt(ElemType i, ElemType elem) //插入元素，  在线性表中第i个位置之前插入新元素elem
{
...
ElemType ChainList::removeAt(ElemType i, ElemType &elemOut)
{
...
ElemType ChainList::getAt(ElemType i, ElemType &elemOut)
{
...
ElemType ChainList::getIndexElemAt(ElemType &i, ElemType elemOut)//在线性表中查找与elem的相等的元素，返回第一个与elem相等元素在线性表中的第几个位置
{
...
ElemType ChainList::destroy()  //单链表销毁
{
	while (m_heap)
	{
...
inline ElemType ChainList::isEmpty()const
{
	return (m_heap->m_data == 0);
}
inline Status ChainList::getLength()const
{          
...
void testChainList()
{
	ChainList myChainList;    //创建一个单链表对象
	...
	while (true)
	{
		...
		case 3:
		{
			...
			{
				cout << "当前单链表为空或者尚未建立，数据获取失败！" << endl;
				myChainList.getLength();
			}
			break;
		}
		case 4:
		{
			...
			cout << "请输入要查找的元素：";
			cin >> findElement;
			if (myChainList.getIndexElemAt(getLocation, findElement))
			{
				cout << "找到了元素" << findElement << "，其在表中的索引值为：" << getLocation << "\n" << endl;
				myChainList.getLength();
				myChainList.show();
			}
			else
